...
let lib = {
	...
	oauth_urac: (config, dataPath, mongoConnection, cb) => {
		let records = [];
		fs.readdirSync(dataPath).forEach(function (file) {
			let rec = require(dataPath + file);
			if (Array.isArray(rec)) {
		...
		if (records && Array.isArray(records) && records.length > 0) {
			async.eachSeries(
				...
				(e, cb) => {
					...
					if (config.delete) {
						mongoConnection.deleteOne("oauth_urac", condition, (error) => {
							if (error) {
								console.log("oauth_urac", error);
								return cb();
							} else {
			...
			async.waterfall([
					...
					function (cb) {
						...
						if (fs.existsSync(dataPath + "oauth/urac/")) {
							...
							return lib.oauth_urac(config, dataPath + "oauth/urac/", mongoConnection, cb);
						} else {
							return cb(null);
						}
					},

	...
	"login": (soajs, inputmaskData, options, cb) => {
		...
		uracDriver.getRecord(soajs, data, function (error, record) {
			...
			if (
				...
				let demoUsernames = [];
				if (soajs.registry.custom.oauth.value.demoAccount.usernames && Array.isArray(soajs.registry.custom.oauth.value.demoAccount.usernames)) {
					demoUsernames = [...soajs.registry.custom.oauth.value.demoAccount.usernames];
				}
				if (soajs.registry.custom.oauth.value.demoAccount.username) {
					demoUsernames.push(soajs.registry.custom.oauth.value.demoAccount.username);
				}
				if (demoUsernames.includes(inputmaskData.usernames)) {
					inputmaskData.code = soajs.registry.custom.oauth.value.demoAccount.code;
				}
			}
			let data = {
				"code": inputmaskData.code || Math.floor(100000 + Math.random() * 900000).toString(),
				"user": record,
				"username": inputmaskData.username,
			...
			if (error || !codeRecord) {
				if (
					...
					if (req.soajs.registry.custom.oauth.value.demoAccount.phones && Array.isArray(req.soajs.registry.custom.oauth.value.demoAccount.phones)) {
						demoUsernames = req.soajs.registry.custom.oauth.value.demoAccount.phones;
					}
					if (req.soajs.registry.custom.oauth.value.demoAccount.phone) {
						demoUsernames.push(req.soajs.registry.custom.oauth.value.demoAccount.phone);
					}
					if (demoUsernames.includes(inputmaskData.phone)) {
						// if (
						// 	req.soajs.registry &&
						// 	req.soajs.registry.custom &&
						// 	req.soajs.registry.custom.oauth &&
